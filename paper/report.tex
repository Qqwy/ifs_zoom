...
\label{sec:org9f223c8}
\label{subsection:viewport_transformation}

When rendering graphics, we view the world through a (virtual) \emph{camera} which has a particular frustrum 
that limits what parts of the world (in this case: the IFS' attractor) ends up on the \emph{viewport}.

\begin{enumerate}
\item Scaling vs zooming
\label{sec:orgf5c8cf7}
...
\item Freedom of choosing an initial camera position and frustrum
\label{sec:org511cb61}
...
Instead, General-Purpose GPU-programming techniques have to be employed, as these are able to use the top-down approach.

\subsubsection{The chaos game on the GPU}
\label{sec:org7561620}
