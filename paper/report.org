...
The stochastic method, more frequently called the /chaos game/, works as seen in autoref:chaosGame
...
- because the precision of the canvas we render on is finite, and because all transformations are contracting,
  only the latest $x$ transformations determine at what location on the canvas a point will end up.[fn:contraction]
